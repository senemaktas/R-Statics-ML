## sources : https://www.youtube.com/watch?v=qcvAqAH60Yw&list=PLblh5JKOoLUICTaGLRoHQDuF_7q2GfuJF&index=7

library(pROC) # install with install.packages("pROC") , it will draw ROC graphs 
library(randomForest) # install with install.packages("randomForest") for classification
...
num.samples <- 100  # samples number

## generate 100 values from a normal distribution with mean 172 and standard deviation 29,then sort them
weight <- sort(rnorm(n=num.samples, mean=172, sd=29))

## Now we will decide if a sample is obese or not. 
## NOTE: This method for classifying a sample as obese or not was made up just for this example.
## rank(weight) returns 1 for the lightest, 2 for the second lightest, ... and it returns 100 for the heaviest.
## So what we do is generate a random number between 0 and 1. Then we see if that number is less than rank/100. 
...
## and classify this sample "obese" if that random number is < 2/100. We repeat that process for all 100 samples
...
## classified "obese" with 1, and which samples were classified "not obese" with 0.

## plot the data
plot(x=weight, y=obese)

## fit a logistic regression to the data... gim.fit is a variable, glm is a function..
## glm function used to fit a logistic regression curve to the data.
glm.fit=glm(obese ~ weight, family=binomial)
## and pass weight and the fitted.values stored in glm.fit into the lines() function to draw curve predicted probabilities...
## glm.fit$fitted.values contains the y-axis coordinates along the curve for each sample...
...
## However, the window in which I draw them isn't square so extra whitespace is added to pad the sides.
...
## If we want to find out the optimal threshold we can store the data used to make the ROC graph in a variable(roc.info)...
...
## add=TRUE means sdd that existing graph
